...
public class UniversalLoadingView extends ViewGroup {
	...
	private String LOAD_FAILED_TIP = "加载失败\n点击重试";
	private String LOAD_EMPTY_TIP = "暂无数据";
	...
	public enum State {
		GONE, LOADING, LOADING_FALIED, LOADING_EMPTY
	}
	...
	public UniversalLoadingView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		TypedArray t = null;
		try {
			t = context.obtainStyledAttributes(attrs, R.styleable.MaterialCircleView, 0, defStyleAttr);
			bGradient = t.getBoolean(R.styleable.MaterialCircleView_bGradient, true);
			circleColor = t.getColor(R.styleable.MaterialCircleView_circleColor1, getResources().getColor(android.R.color.holo_blue_light));
			circleWidth = t.getDimensionPixelSize(R.styleable.MaterialCircleView_circleWidth, 10);
			radius = t.getDimensionPixelSize(R.styleable.MaterialCircleView_radius1, MaterialCircleView.dpToPx(50, getResources()));
		} finally {
			if (t != null) {
				t.recycle();
			}
		...
		try {
			t = context.obtainStyledAttributes(attrs, R.styleable.UniversalLoadingView, 0, defStyleAttr);
			setbTransparent(t.getBoolean(R.styleable.UniversalLoadingView_bg_transparent, false));
			alpha = t.getDimensionPixelSize(R.styleable.UniversalLoadingView_bg_alpha, 255);
		} finally {
			if (t != null) {
				t.recycle();
			}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		LayoutParams params = (LayoutParams) materialCircleView.getLayoutParams();
		materialCircleView.layout(params.left, params.top, params.left + params.width, params.top + params.height);
		LayoutParams tipParams = (LayoutParams) mTipTextView.getLayoutParams();
		mTipTextView.layout(tipParams.left, tipParams.top, tipParams.left + tipParams.width, tipParams.top + tipParams.height);
	}
	...
	public void setLoadingFail(String content) {
		LOAD_FAILED_TIP = content;
	}

	public void setloadingEmpty(String content) {
		LOAD_EMPTY_TIP = content;
	}
